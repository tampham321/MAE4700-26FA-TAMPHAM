...
def main():
    ...
    reaction_values = reactions.reshape(
        (num_nodes, ndim)
    )

    # Output files go in same folder as this Python file
    output_folder = Path(__file__).parent

    write_output_file(
        output_folder / "nodal_displacements.txt",
        nodal_displacements.tolist()
    )

    write_output_file(
        output_folder / "reaction_forces.txt",
        reaction_values.tolist()
    )

    write_output_file(
        output_folder / "internal_forces.txt",
        internal_forces
    )

    print("\nOutput files created:")
    print(output_folder / "nodal_displacements.txt")
    print(output_folder / "reaction_forces.txt")
    print(output_folder / "internal_forces.txt")

    print("\nNodal displacements:")
    print(nodal_displacements.tolist())

    print("\nReaction forces:")
    print(reaction_values.tolist())
